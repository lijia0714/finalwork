...
class newqLearning:
    levelAccuracy = [0.98, 0.95, 0.90, 0.85, 0.75, 0.55, 0.30, 0.10, 0.0]
    ...
    rewardTable = [[0 for i in range(int(totalNum / 2))]
                   for j in range(len(state))]

    rateExplore = None      # # e-greedy
    currentState = None
    # currentReward = 0
    currentAction = None
    ...
    def updateState(self, acc, boolDownload):
        ...
        return self.state[(acc, boolDownload)]

    # 计算奖励值
    def caculateReward(self, next_state, boolDown):
        # # 如果没有从后台下载
        if not boolDown:
            reward = 63 * (totalNum + 1 - self.currentAction) * (20 - next_state)
        else:
            reward = (-64) * (self.currentAction + 1) * (next_state)
        ...
        qsa_2 = self.rewardTable[next_state][self.rewardTable[next_state].index(
            max(self.rewardTable[next_state]))]
    ...
    def run(self, x, r, label):
        if r.isFull():
            train_x, train_y = r.withDrawData()
            if self.acc_li[-1] > 0.98:
            ...
            self.acc_li.append(self.eva(dataTrue, dataPred))
            print("acc:  ", self.acc_li[-1], "\n")

        # 动作选择，选择当前的 action，并由 x 预测 pred(1000*10*(a,b,c))
        self.makeAct()

        pred = self.resModel.predict(x)
        pred = self.edgeProcessed(pred, self.currentAction)
        # # 对 label 进行处理
        ...
        next_state = self.updateState(acc, boolDown)
...
class Environment:
    ...
    def getEvaluateResult(self, dataTrue, dataPred):
        ...
        sumNeed = 0.0       # # 实际下载的块大小
        sumDownload = 0.0   # # pred 下载的块大小
        sumAimed = 0.0      # # 重合的块大小
        ...
        self.acc_li.append(rateDownload)
        print('rateNeed: ', rateNeed, 'rateDown: ', rateDownload)
        if rateNeed >= 0.98:
...
if __name__ == '__main__':
    ...
    for i in range(1000):
        ...
        # # 由 x, label 传入进行训练
        boolDown, acc = q.run(x=x, r=r, label=label)
        # 如果不从backhalu中下载的话
        if not boolDown:
